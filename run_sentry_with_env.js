...
if (!fs.existsSync(envPath)) {
  console.error("vercel.env not found at", envPath);
  process.exit(2);
}
...
console.log(
  "Loaded environment variables from vercel.env (hiding token values).",
);
[
  "SENTRY_AUTH_TOKEN",
  "SENTRY_ORG",
  ...
  if (process.env[k])
    console.log(k + ":", k.includes("TOKEN") ? "<redacted>" : process.env[k]);
});
...
if (
  ...
  try {
    ...
    if (sha) {
      ...
      console.log(
        "Detected local git commit SHA and set VERCEL_GIT_COMMIT_SHA:",
        sha,
      );
    }
  } catch (e) {
...
const child = spawn(process.execPath, ["scripts/sentry-postbuild.js"], {
  stdio: "inherit",
  env: process.env,
});
child.on("exit", (code) => process.exit(code));
child.on("error", (err) => {
  console.error("Uploader spawn error:", err);
  process.exit(3);
});
