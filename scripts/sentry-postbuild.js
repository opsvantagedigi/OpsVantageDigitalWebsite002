...
if (!SENTRY_AUTH_TOKEN || !SENTRY_ORG || !SENTRY_PROJECT || !COMMIT) {
  console.log(
    "Sentry upload skipped: missing one of SENTRY_AUTH_TOKEN, SENTRY_ORG, SENTRY_PROJECT, or commit SHA.",
  );
  console.log(
    "To enable automatic source-map upload, set these env vars in Vercel (production) or create a local .sentry.env file for testing:",
  );
  console.log(
...
try {
  ...
  execSync(
    `npx sentry-cli releases -o ${SENTRY_ORG} -p ${SENTRY_PROJECT} new ${COMMIT}`,
    { stdio: "inherit" },
  );
  ...
  execSync(
    `npx sentry-cli releases -o ${SENTRY_ORG} -p ${SENTRY_PROJECT} finalize ${COMMIT}`,
    { stdio: "inherit" },
  );
