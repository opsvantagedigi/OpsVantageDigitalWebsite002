...
export function NavigationMenuItem({ href, children, className }) {
  return (
    <Link
      href={href}
      className={cn(
        "text-sm font-medium text-muted-2 hover:text-white px-2 py-1",
        className,
      )}
    >
      {children}
...
export function NavigationMenuDropdown({ label, children, className }) {
  return (
    <details className={cn("relative", className)}>
      <summary
        className="cursor-pointer list-none text-sm font-medium text-muted-2 px-2 py-1"
        aria-haspopup="true"
      >
        {label}
      </summary>
      <div
        className="absolute z-20 mt-2 w-48 rounded-md border bg-[var(--marz)] shadow-lg"
        role="menu"
        aria-label={label}
      >
